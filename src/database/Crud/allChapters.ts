export async function createChapterAll(
...
export function getAllChaptersAll(realm: any) {
  return realm.objects("AllChapter");
}

export function deleteChapterByUidAll(realm: any, uid: string) {
  try {
    // Find the chapter object using its primary key (uid)
    const chapterToDelete = realm.objectForPrimaryKey("AllChapter", uid);
...
export function deleteAllChaptersAll(realm: any): boolean {
  try {
    // Obtém todos os objetos da coleção "Chapter"
    const allChapters = realm.objects("AllChapter");
...
export async function toggleChapter(
  ...
  try {
    ...
    const existingChapterResults = getChapterAll(
      realm,
      chapterData.idFont,
      chapterData.idManga,
      chapterData.idChap
    ...
    if (existingChapterResults.length > 0) {
      ...
      const deleted = deleteChapterByUidAll(realm, uid); // Use sua função existente
      if (deleted) {
      ...
      await createChapterAll(realm, chapterData); // Use sua função existente
